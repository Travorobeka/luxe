class Megamenu {
  ...
  init() {
    this.domNodes.hamburgerButtons.addEventListener("click", (e) => {
      ...
      const anim = btn.getAttribute('data-anim') || 'fade';
      const hamburgerIcon = btn.querySelector('.m-hamburger-icon[data-state="hamburger"]');
      const closeIcon = btn.querySelector('.m-hamburger-icon[data-state="close"]');
      if (hamburgerIcon && closeIcon) {
        if (isActive) {
          // Animate out hamburger, in close
          if (anim === 'fade') {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => { closeIcon.style.opacity = 1; }, 10);
            }, 200);
          } else if (anim === 'rotate') {
            hamburgerIcon.style.transition = 'transform 0.3s, opacity 0.2s';
            closeIcon.style.transition = 'transform 0.3s, opacity 0.2s';
            ...
            // For morph, just crossfade for now (real morph would require SVG path animation)
            hamburgerIcon.style.transition = 'opacity 0.3s';
            closeIcon.style.transition = 'opacity 0.3s';
            hamburgerIcon.style.opacity = 0;
            setTimeout(() => {
              hamburgerIcon.style.display = 'none';
              closeIcon.style.display = 'block';
          ...
          // Animate out close, in hamburger
          if (anim === 'fade') {
            closeIcon.style.transition = 'opacity 0.2s';
            hamburgerIcon.style.transition = 'opacity 0.2s';
            closeIcon.style.opacity = 0;
            setTimeout(() => {
              ...
              hamburgerIcon.style.opacity = 0;
              setTimeout(() => { hamburgerIcon.style.opacity = 1; }, 10);
            }, 200);
          } else if (anim === 'rotate') {
            closeIcon.style.transition = 'transform 0.3s, opacity 0.2s';
            hamburgerIcon.style.transition = 'transform 0.3s, opacity 0.2s';
